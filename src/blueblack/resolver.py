...
class TimeResolver:
    ...
    def __init__(
        self, lat: str, lng: str, timezone: str, offsetSunrise: str, offsetSunset: str
    ) -> None:
        self.lat = lat
        self.lng = lng
    ...
    def calculate(
        self, latitude: str, longitude: str, timezone: str, offsetSunrise: str, offsetSunset: str
    ) -> SunTimes:
        ...
        return SunTimes(
            (
                s["sunrise"]
                + timedelta(
                    hours=offsetSunriseTimes[0],
                    minutes=offsetSunriseTimes[1],
                    seconds=offsetSunriseTimes[2],
                )
            ).time(),
            (
                s["sunset"]
                + timedelta(
                    hours=offsetSunsetTimes[0],
                    minutes=offsetSunsetTimes[1],
                    seconds=offsetSunsetTimes[2],
                )
            ).time(),
        )

    def resolve(self) -> SunTimes:
        return self.calculate(
            self.lat, self.lng, self.timezone, self.offsetSunrise, self.offsetSunset
        )
